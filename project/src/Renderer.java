...
public class Renderer {
    ...
    private static final int GRID_PRERENDER_X = 1200;
    private static final int GRID_PRERENDER_Y = 1200;
    ...
    // player highlight appearance
    private static final Color PLAYER_HIGHLIGHT_COLOR = Color.RED;
    private static final int HIGH_RADIUS = 15;
    private static final int HIGH_DURATION = 100;

    // whether to wipe the canvas clean on each frame (recommended)
    private static final boolean CLEAN_FRAME = true;

    private static final Font stringFont = new Font("SansSerif", Font.PLAIN,
            36);
    private static final Font selectedFont = new Font("Helvetica", Font.BOLD,
            52);
    private static final Font inGameFont = new Font("SansSerif", Font.BOLD, 24);
    private static final Font gameOverFont = new Font("SansSerif", Font.BOLD,
            72);
    ...
    public void drawGrid(Grid grid) {
        ...
        for (Entity ent : grid.getEntities()) {
            g.drawImage(ent.getSprite().getCurrentImage(),
                    (int) (ent.getX() * TX), (int) (ent.getY() * TY), TX, TY,
                    null);
        }
        ...
        if (grid.getPlayer2() != null) {
            p2 = grid.getPlayer2();
            g.drawImage(p2.getSprite().getCurrentImage(),
                    (int) (p2.getX() * TX), (int) (p2.getY() * TY), TX, TY,
                    null);
        }
        ...
        if (grid.getPlayer2() == null) {
            ...
            g.drawImage(p.getSprite().getCurrentImage(), -182, S * 7 / 32, 64,
                    64, null);
            drawStringCentred(g, "Coins Left", -150, S / 2);
            drawStringCentred(g, Integer.toString(grid.getCoinsLeft()), -150,
                    S * 7 / 12);
        } else {
            // Prints image of both players, their coins collected and total
            // coins left
            drawStringCentred(g, "Player 1", -150, S / 8);
            g.drawImage(p.getSprite().getCurrentImage(), -182, S * 5 / 32, 64,
                    64, null);
            drawStringCentred(g, "Coins Collected", -150, S * 8 / 32);
            drawStringCentred(g, Integer.toString(grid.getCoinsCollected()),
                    -150, S * 10 / 32);
            drawStringCentred(g, "Player 2", -150, S * 13 / 32);
            g.drawImage(p2.getSprite().getCurrentImage(), -182, S * 14 / 32, 64,
                    64, null);
            drawStringCentred(g, "Coins Collected", -150, S * 17 / 32);
            drawStringCentred(g, Integer.toString(grid.getCoinsCollectedTwo()),
                    -150, S * 19 / 32);
        ...
        if (grid.getUpdateCount() < HIGH_DURATION) {
            ...
            final int rad = (HIGH_DURATION - grid.getUpdateCount()) * S
                    * HIGH_RADIUS / 100 / HIGH_DURATION;
            g.drawOval((int) ((p.getX() + .5f) * TX - rad),
                    (int) ((p.getY() + .5f) * TY - rad), rad * 2, rad * 2);
            if (grid.getPlayer2() != null) {
                p2 = grid.getPlayer2();
                g.drawOval((int) ((p2.getX() + .5f) * TX - rad),
                        (int) ((p2.getY() + .5f) * TY - rad), rad * 2, rad * 2);
            }
        }
    }
    ...
    /**
     * Draws the end state, either player has won, player has lost, multiplayer
     * - player 1 has won or player 2 has won. Depending on whether all the
     * coins have been collected, in singleplayer the player has won and in
     * multiplayer the player with most coins out of the two win Otherwise if
     * the AI has collided with a player, in singleplayer the player loses and
     * in multiplayer the play not killed by the AI wins.
     * 
     * The first if statement will print out either Game over or Congratulations
     * player has one depending on how the game has ended
     * 
     ...
     */
    public void drawEndState(EndState end, boolean multiplayer, int gameEndMode,
            int coinsCollected, int coinsCollected2, int coinsLeft) {
        final int SX = 1600;
        final int SY = 900;
        ...
        if (coinsLeft == 0) {
            ...
            if (multiplayer == true && coinsCollected > coinsCollected2) {
                drawStringCentred(g, "CONGRATULATIONS PLAYER 1 HAS WON", SX / 2,
                        SY / 8);
                g.drawImage(ImageCache.getImage("wonImage"), (SX / 2 - 32),
                        SY * 3 / 16, 128, 128, null);
                drawStringCentred(g,
                        "Coins Collected: " + Integer.toString(coinsCollected),
                        SX / 2, SY * 2 / 5);
                // player 2 wins
            } else if (multiplayer == true
                    && coinsCollected < coinsCollected2) {
                drawStringCentred(g, "CONGRATULATIONS PLAYER 2 HAS WON", SX / 2,
                        SY / 8);
                g.drawImage(ImageCache.getImage("win2"), (SX / 2 - 32),
                        SY * 3 / 16, 128, 128, null);
                drawStringCentred(g,
                        "Coins Collected: " + Integer.toString(coinsCollected2),
                        SX / 2, SY * 2 / 5);
            } else {
                // Single player collected all coins
                drawStringCentred(g, "CONGRATULATIONS YOU HAVE WON", SX / 2,
                        SY / 4);
            }
        } else {
            // Otherwise game over and prints the number of coins collected
            // If is multi player and player 1 has more coins collected or
            // player 2 has been killed by AI
            if (multiplayer == true && gameEndMode == 2) {
                ...
                drawStringCentred(g,
                        "Coins Collected: " + Integer.toString(coinsCollected),
                        SX / 2, SY * 2 / 5);
                // If is multi player and player 2 has more coins collected or
                // player 1 has been killed by AI
                ...
                g.drawImage(ImageCache.getImage("youLost"), (SX / 2 - 32),
                        SY * 3 / 16, 128, 128, null);
                drawStringCentred(g,
                        "Coins Collected: " + Integer.toString(coinsCollected),
                        SX / 2, SY * 2 / 5);
    ...
    public void drawMenu(Menu m) {
        ...
        Graphics2D g = getTransformedGraphics(MENU_MARGIN, SX, SY);
        // Draws Title image
        g.drawImage(ImageCache.getImage("title"), SX / 4, 0, SX / 2, SY / 2,
                null);
        // Strings of what is to be printed to main menu
        String[] difficulty = new String[] { "SINGLE PLAYER:EASY",
                "SINGLE PLAYER:MEDIUM", "SINGLE PLAYER:HARD" };
        String[] strings = new String[] { difficulty[m.getDifficulty()],
                "MULTIPLAYER", "INSTRUCTIONS", "QUIT" };
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (i == 0) {
                ...
                drawButton(m, 0, g, 0, buttonY, SX / 8, SY / 8, colFill,
                        colContent, "<", textFont);

                drawButton(m, 1, g, SX / 8, buttonY, SX * 6 / 8, SY / 8,
                        colFill, colContent, strings[i], textFont);

                drawButton(m, 2, g, SX * 7 / 8, buttonY, SX / 8, SY / 8,
                        colFill, colContent, ">", textFont);
            } else {
                // Drawing the other 3 rows of buttons
                drawButton(m, i + 2, g, 0, buttonY, SX, SY / 8, colFill,
                        colContent, strings[i], textFont);
            }
        }
    }
     ...
     */
    private void drawButton(Menu m, int mIndex, Graphics2D g, int x, int y,
            int w, int h, Color cf, Color cc, String text, Font font) {
        g.setColor(cf);
        g.fillRect(x, y, w, h);
        g.setColor(cc);
        g.drawRect(x, y, w, h);
        ...
        g.getTransform().transform(pts, 0, pts, 0, pts.length / 2);
        m.setMouseArea(mIndex, (int) pts[0], (int) pts[1], (int) pts[2],
                (int) pts[3]);
    }
